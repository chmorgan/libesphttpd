...
CgiStatus ICACHE_FLASH_ATTR cgiEspVfsHook(HttpdConnData *connData) {
	...
	if (file==NULL) {
		if (connData->cgiArg != NULL) {
			strcpy(filename, connData->cgiArg);
		} 
		strcat(filename, connData->url);
...
//cgiEspVfsTemplate can be used as a template.
...
CgiStatus ICACHE_FLASH_ATTR cgiEspVfsTemplate(HttpdConnData *connData) {
	...
	len=fread(buff, 1, 1024, tpd->file);
	if (len>0) {
		sp=0;
		e=buff;
		for (x=0; x<len; x++) {
...
CgiStatus   cgiEspVfsUpload(HttpdConnData *connData) {
	...
	if (file==NULL) {
        ...
        if (connData->requestType!=HTTPD_METHOD_PUT) {
            ...
            strcpy(filename, connData->cgiArg);
            strcat(filename, "/");
            strcat(filename, connData->getArgs);
    ...
    if (connData->post.received == connData->post.len) {
        ...
        httpdSend(connData, "{", -1);
        len = sprintf(output, "\"filename\": \"%s/%s\", ", (char *) connData->cgiArg, (char *) connData->getArgs);
        httpdSend(connData, output, len);
        len = sprintf(output, "\"size\": \"%d\" ", connData->post.received);
        httpdSend(connData, output, len);
        httpdSend(connData, "}", -1);
        return HTTPD_CGI_DONE;
	} else {
