...
CgiStatus cgiGetFirmwareNext(HttpdConnData *connData) {
	...
	ESP_LOGD(__func__, "Next firmware: %s (got %d)", next, id);
	return HTTPD_CGI_DONE;
}
...
CgiStatus cgiUploadFirmware(HttpdConnData *connData) {
	...
	if (state == NULL) {
		...
		ESP_LOGD(__func__, "Firmware upload cgi start");
		state = malloc(sizeof(UploadState));
		if (state==NULL) {
			ESP_LOGE(__func__, "Can't allocate firmware upload struct");
			return HTTPD_CGI_DONE;
		}
		...
		{
			ESP_LOGE(__func__, "configured or running parititon is null, is OTA support enabled in build configuration?");
			state->state=FLST_ERROR;
			state->err="Partition error, OTA not supported?";
		} else {
			if (state->configured != state->running) {
				ESP_LOGW(__func__, "Configured OTA boot partition at offset 0x%08x, but running from offset 0x%08x",
					state->configured->address, state->running->address);
				ESP_LOGW(__func__, "(This can happen if either the OTA boot data or preferred boot image become corrupted somehow.)");
			}
			ESP_LOGI(__func__, "Running partition type %d subtype %d (offset 0x%08x)",
...
//// HTTP GET queryParameter "partition" : string
	    ...
	    {
			ESP_LOGE(__func__, "update_partition not found!");
	...
	while (dataLen!=0) {
		if (state->state==FLST_START) {
			...
			if (def->type==CGIFLASH_TYPE_FW && memcmp(data, "EHUG", 4)==0) {
				...
				state->state=FLST_ERROR;
				ESP_LOGE(__func__, "Combined flash image not supported on ESP32!");
			    ...
			    {
			    	ESP_LOGI(__func__, "Writing to partition subtype %d at offset 0x%x", state->update_partition->subtype, state->update_partition->address);
				...
				state->state=FLST_ERROR;
				ESP_LOGE(__func__, "Did not recognize flash image type");
			}
		} else if (state->state==FLST_WRITE) {
			err = esp_ota_write(state->update_handle, data, dataLen);
			if (err != ESP_OK) {
				ESP_LOGE(__func__, "Error: esp_ota_write failed! err=0x%x", err);
			...
			ESP_LOGE(__func__, "%d bogus bytes received after data received", dataLen);
			//Ignore those bytes.
			dataLen=0;
	...
	if  (connData->post.len == connData->post.received) {
		...
		if (state->state==FLST_DONE) {
			if (esp_ota_end(state->update_handle) != ESP_OK) {
		        ...
		        ESP_LOGE(__func__, "esp_ota_end failed!");
		        state->state=FLST_ERROR;
		    }
			else
			{
				state->err="Flash Success.";
				ESP_LOGI(__func__, "Upload done. Sending response");
			}
			// todo: automatically set boot flag?
			err = esp_ota_set_boot_partition(state->update_partition);
			if (err != ESP_OK) {
				ESP_LOGE(__func__, "esp_ota_set_boot_partition failed! err=0x%x", err);
			}
		}
...
CgiStatus cgiRebootFirmware(HttpdConnData *connData) {
	...
	ESP_LOGD(__func__, "Reboot Command recvd. Sending response");
...
//// HTTP GET queryParameter "partition" : string
    ...
    if (len > 0)
    {
    	ESP_LOGD(__func__, "Set Boot Command recvd. for partition with name: %s", arg_partition_buf);
...
//// HTTP GET queryParameter "partition" : string
    ...
    if (len > 0)
    {
    	ESP_LOGD(__func__, "Erase command recvd. for partition with name: %s", arg_partition_buf);
    	...
    	if (err != ESP_OK) {
    		ESP_LOGE(__func__, "erase partition failed! err=0x%x", err);
    	}
    	else
    	{
    		ESP_LOGW(__func__, "Data partition: %s is erased now!  Must reboot to reformat it!", wanted_partition->label);
